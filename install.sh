#!/usr/bin/env bash
### wget -q https://raw.githubusercontent.com/thanhphatit/cicd-manager/main/install.sh -O - | /bin/bash
...
GITHUB_ORG=thanhphatit
GITHUB_REPO=cicd-manager
...
function verifySupported() {
    ...
    if [ "$current_osarch" == "darwin-arm64" ]; then
        if isReleaseAvailable $RELEASE_TAG; then
            return
        else
